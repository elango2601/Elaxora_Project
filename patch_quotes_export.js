const fs = require('fs');
const file = 'frontend/src/app/admin/quotes/page.tsx';
...
code = code.replace(
  /<div>\n\s*<span className="text-\[10px\] font-bold text-indigo-400 uppercase tracking-widest">Financial Desk<\/span>\n\s*<h1 className="text-2xl font-extrabold text-foreground tracking-tight mt-1">Quotations Logs<\/h1>\n\s*<p className="text-muted text-xs">Review generated custom invoices, pricing, and student response statuses\.<\/p>\n\s*<\/div>/,
  headerUI
);

fs.writeFileSync(file, code);
console.log('patched quotes export');
